...
namespace exafmm {
 ...
 //    };
  ...
  Tracer tracer;                                          //  Instantiate tracer
  logger::startTracer(tracer);                            //  Start tracer
  if (CiEnd - CiBegin == 3 || CjEnd - CjBegin == 3) {     //  If only 3 cells in range
    if (CiBegin == CjBegin) {                             //   If Ci == Cj
      assert(CiEnd == CjEnd);                             //    Check if mutual & self interaction
      traversal->dualTreeTraversal(CiBegin, CjBegin, mutual, remote);//   Call traverse for single pair
    } else {                                              //   If Ci != Cj
    ...
    C_iter stepCi = (CiEnd - CiBegin) / 4;
    C_iter stepCj = (CjEnd - CjBegin) / 4;
    
    C_iter CiQuad1 = CiBegin + stepCi;
    C_iter CiQuad2 = CiQuad1 + stepCi;
    C_iter CiQuad3 = CiQuad2 + stepCi;

    C_iter CjQuad1 = CjBegin + stepCj;
    C_iter CjQuad2 = CjQuad1 + stepCj;
    C_iter CjQuad3 = CjQuad2 + stepCj;
    
    mk_task_group;                                    //   Initialize task group
    {
      TraverseRange brach1(traversal, CiBegin, CiQuad1,//    Instantiate recursive functor
             CjBegin, CjQuad1, mutual, remote);
      create_taskc(brach1);                           //    Ci:former Cj:former
      
      TraverseRange brach2(traversal, CiQuad1, CiQuad2, //    Instantiate recursive functor
             CjQuad1, CjQuad2, mutual, remote);
      create_taskc(brach2);                           //    Ci:former Cj:former
      
      TraverseRange brach3(traversal, CiQuad2, CiQuad3, //    Instantiate recursive functor
        CjQuad2, CjQuad3, mutual, remote);
      create_taskc(brach3);                           //    Ci:former Cj:former

      TraverseRange brach4(traversal, CiQuad3, CiEnd, //    Instantiate recursive functor
        CjQuad3, CjEnd, mutual, remote);
      brach4();
      wait_tasks;
    }
    {
