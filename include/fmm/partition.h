...
public:
  ...
  struct bodyComparer {
    bool operator() (Body i,Body j) { return (i.ICELL<j.ICELL); }
  } comparer;


#if EXAFMM_USE_PARMETIS
  void partitionParmetis(Bodies & bodies, Bounds globalBounds) { 
    logger::startTimer("Partition");                          // Start timer
    //int64_t ndims = 3;
    int ndims = 3;
    //std::vector<double> xyz(bodies.size()*ndims);
    std::vector<float> xyz(bodies.size()*ndims);
    for(int i = 0; i < bodies.size(); ++i) {
      xyz[i*ndims + 0] = bodies[i].X[0];
      xyz[i*ndims + 1] = bodies[i].X[1];
      xyz[i*ndims + 2] = bodies[i].X[2];
    }
    std::vector<int> part(bodies.size());
    MPI_Comm comm = MPI_COMM_WORLD;
    size_t bodyCount = bodies.size();
    size_t* counts = new size_t[mpisize];
    MPI_Allgather(&bodyCount, 1, MPI_UNSIGNED_LONG_LONG,
                  counts,     1, MPI_UNSIGNED_LONG_LONG,
                  MPI_COMM_WORLD);
    if(bodyCount > 0) 
      parmetis_wrapper::partitionCoodsParmetis(counts, &xyz[0], &part[0], &ndims, mpisize, &comm);
    else 
      parmetis_wrapper::partitionCoodsParmetis(counts, NULL, &part[0], &ndims, mpisize, &comm);

    for(int i = 0; i < bodies.size(); ++i) {
      bodies[i].IRANK = part[i];
    }
    delete[] counts;
  ...
  void partitionHilbert(Bodies & bodies, Bounds globalBounds) {
    ...
    if (mpisize > 1) {
      ...
      uint64_t lbound = globalKeyBounds.first;
      uint64_t rbound = globalKeyBounds.second;
      VHilbert rq;
...
//      keys.reserve(bodies.size());
      int index = 0;
      logger::startTimer("Sort Hilbert");
      std::sort(bodies.begin(), bodies.end(), comparer);
      logger::stopTimer("Sort Hilbert");
      
      if(bodies.size() > 0) {  
        uint64_t prev_icell = bodies.begin()->ICELL;
        for (B_iter B = bodies.begin()+1; B != bodies.end(); ++B){
           if(B->ICELL == prev_icell) {
             int inc = 1;
             B->ICELL+=1;
             B_iter B0 = B+1;
             while(B0 < bodies.end() && B0->ICELL == prev_icell){
               inc++;
               B0->ICELL+=inc;
               B0++;
             }
   ...
   //   std::cout << "number of collisions: " << index << std::endl;
    } else {
      for (B_iter B = bodies.begin(); B != bodies.end(); ++B)
        B->IRANK = mpirank;
    }
  ...
  Bounds partition(Bodies& bodies, Bounds globalBounds, const char* partitioingType) {
    ...
    switch (partitioingType[0]) {
      ...
      case 'p':
#if EXAFMM_USE_PARMETIS
        partitionParmetis(bodies, globalBounds);
      break;
#else 
        std::cout << "Warning: required parmetis partitioning is not supported" << std::endl;
#endif
      default:
        localBounds = bisection(bodies, globalBounds);
    }
