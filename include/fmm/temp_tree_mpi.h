...
private:
  ...
  void alltoallv(Cells & cells) {
    ...
    std::vector<size_t> check(4); 
    for (int irank = 0; irank < mpisize; irank++) {           // Loop over ranks
      check[0] += word * sendCellCount[irank];
      assert(check[0] < (INT_MAX/4));
      sendCellCount[irank] *= word;                           //  Multiply send count by word size of data
      
      check[1] += word * sendCellDispl[irank];
      assert(check[1] < (INT_MAX/4));
      sendCellDispl[irank] *= word;                           //  Multiply send displacement by word size of data



      check[2] += word * recvCellCount[irank];
      assert(check[2] < (INT_MAX/4));
      recvCellCount[irank] *= word;                           //  Multiply receive count by word size of data
      

      check[3] += word * recvCellDispl[irank];
      assert(check[3] < (INT_MAX/4));
